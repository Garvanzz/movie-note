...
const mockHandlers: Record<string, (args: Record<string, unknown>) => unknown> = {
  ...
  get_actor_names: (args) => [
    {
      id: 1,
      actor_id: args.actorId as number,
      name: "Mock Actor",
      kind: "native",
      is_primary: true,
      sort_order: 0,
  ...
  add_actor_name: (args) => ({
    ...
    kind: (args.kind as string) ?? "alias",
    is_primary: !!args.isPrimary,
    sort_order: 0,
  }),
  add_actor_alias: () => undefined,
  update_actor_name: (args) => ({
  ...
  merge_actors: (args) => ({
    id: args.targetActorId as number,
    name: "Merged Actor",
    name_jp: null,
    measurements: null,
    birth_date: null,
